"""Y Combinator (Work at a Startup) feed slice.
...
from jobtracker.core.config import load_criteria
...
def _enrichment_keywords() -> list[str]:
    ...
    try:
        return list(load_criteria().get("positive_title_keywords", {}).keys())
    except Exception:
...
def scrape(source: dict) -> list[dict]:
    ...
    source_name = source.get("name", "Y Combinator Jobs")
    listings = []
    try:
